# ☕ Oracle Certified Professional: Java SE 21 Developer (1Z0-830)

Repository này lưu trữ toàn bộ source code, bài tập thực hành và ghi chú cá nhân trong quá trình tôi hoàn thành khóa học luyện thi chứng chỉ Java SE 21 Developer.

## 🎯 Mục tiêu
- **Chứng chỉ:** Oracle Certified Professional: Java SE 21 Developer (1Z0-830)
- **Tiến độ khóa học:** Đang thực hiện 🚀

## 📚 Lộ trình học tập & Tiến độ (Course Progress)

### Part 1: Java Basics & OOP
- [X] **Section 1:** Introduction to Java SE 21 Developer Certification Course
- [X] **Section 2:** Introduction to Java and Environment Setup
- [X] **Section 3:** Basics of Java
- [X] **Section 4:** Operators
- [ ] **Section 5:** Array
- [ ] **Section 6:** Control Flow Statements
- [ ] **Section 7:** Methods
- [ ] **Section 8:** Java Object-Oriented Concept
- [ ] **Section 9:** Java Object-Oriented Programming

### Part 2: Core Java APIs & Functional Programming
- [ ] **Section 10:** Wrapper Classes, Auto-Boxing and Unboxing
- [ ] **Section 11:** Strings
- [ ] **Section 12:** Collections
- [ ] **Section 13:** Exception Handling in Java
- [ ] **Section 14:** Enum Types
- [ ] **Section 15:** Lambda Expression
- [ ] **Section 16:** Stream API
- [ ] **Section 17:** Date & Time
- [ ] **Section 18:** Java IO (Input / Output)

### Part 3: Concurrency & Multithreading (Deep Dive)
- [ ] **Section 19:** Overview of Threads
- [ ] **Section 20:** Thread Creation and Management
- [ ] **Section 21:** Synchronization and Concurrency Control
- [ ] **Section 22:** High-level Concurrency APIs
- [ ] **Section 23:** Thread Coordination
- [ ] **Section 24:** Locks and Advanced Synchronization
- [ ] **Section 25:** Concurrent Collections
- [ ] **Section 26:** Performance and Scalability
- [ ] **Section 27:** Java Virtual Machine and Threads
- [ ] **Section 28:** Best Practices and Advanced Topics

### Part 4: Practical Application & Exam Prep
- [ ] **Section 29:** Case Studies and Practical Scenarios
- [ ] **Section 30:** Tools and Testing
- [ ] **Section 31:** Mock Exam - Java SE 21 Developer Certification
- [ ] **Section 32:** Practice Exam
- [ ] **Section 33:** Extra

## 🛠️ Cấu trúc thư mục
Code được tổ chức theo từng section tương ứng để dễ dàng tra cứu:

```text
/src
  ├── /section-03-basics
  ├── /section-09-oop
  ├── /section-16-streams
  ├── /section-21-concurrency
  ...
```
## Documents
https://docs.google.com/document/d/108gStTYzg6rXPNr9yljmAQQ-xhbHiUlX7YG89O2pZzY/edit?usp=sharing
